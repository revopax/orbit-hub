...
export default function LinkedInAds({ accent, secondary }: Props) {
  const [data, setData] = useState<any>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/data/linkedin_organico.json')
      .then(r => r.json())
  ...
  return (
    <div style={{ padding:24, maxWidth:1400, margin:'0 auto' }}>
      <div style={{ background:`linear-gradient(135deg,${accent}22,${secondary}11)`, border:`1px solid ${accent}33`, borderRadius:16, padding:'20px 24px', marginBottom:24, display:'flex', alignItems:'center', gap:16 }}>
        <img src="/logos/Linkedin-Ads-logo.png" alt="LinkedIn Ads" style={{ width:48, height:48, objectFit:'contain', borderRadius:12, background:'white', padding:4 }}/>
        <div>
          <div style={{ fontSize:18, fontWeight:700, color:'#1e293b' }}>LinkedIn Ads</div>
          <div style={{ fontSize:13, color:'#64748b' }}>LinkedIn Marketing · Campañas Pagadas</div>
        </div>
      </div>
