...
export default function BloqueCalidadLeads({ selectedUDNs, data }: BloqueCalidadLeadsProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 mb-6">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="text-left py-2 px-2 text-slate-500 font-medium text-xs">UDN</th>
              <th className="text-center py-2 px-2 text-slate-500 font-medium text-xs">Score Adecuación</th>
              <th className="text-center py-2 px-2 text-slate-500 font-medium text-xs">Score Interacción</th>
              <th className="text-center py-2 px-2 text-slate-500 font-medium text-xs">Leads totales</th>
            </tr>
          </thead>
          <tbody>
            {qualities.map(q => {
              ...
              return (
                <tr key={q.udn} className="border-b border-slate-50 hover:bg-slate-50">
                  ...
                  <td className="py-2 px-2 text-center">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className={`font-semibold ${isBestAde ? 'text-emerald-600' : 'text-slate-600'}`}>
      ...
      <div className="mt-4">
        <p className="text-xs font-medium text-slate-500 mb-2">Distribución de segmentos lead score</p>
            ...
            return (
              <div key={q.udn}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs text-slate-400">{total.toLocaleString()} leads</span>
                </div>
