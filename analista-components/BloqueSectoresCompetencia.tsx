...
export default function BloqueSectoresCompetencia({ selectedUDNs, data }: BloqueSectoresCompetenciaProps) {
  ...
  const tempBadge = (temp: string) => {
    const colors: Record<string, string> = {
      ...
      templado: 'bg-amber-50 text-amber-700 border-amber-200',
      tibio: 'bg-sky-50 text-sky-700 border-sky-200',
    ...
    return (
      <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-semibold border ${colors[temp] || colors.tibio}`}>
        {labels[temp] || temp}
      </span>
    )
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 mb-6">
      ...
      <p className="text-xs text-slate-500 mb-4">
        {sectoresMatrix.length} sectores con solapamiento — temperatura IGAE + mes pico
      </p>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="text-left py-2 px-2 text-slate-500 font-medium">Sector</th>
              {selectedUDNs.map(id => (
          ...
          <tbody>
            {sectoresMatrix.map(sector => (
              <tr key={sector.nombre} className="border-b border-slate-50 hover:bg-slate-50">
                <td className="py-2 px-2">
                  <div className="font-medium text-slate-700">{sector.nombre}</div>
                </td>
                {selectedUDNs.map(id => {
                    ...
                    <td key={id} className="py-2 px-2 text-center">
                      {info ? (
                        <div className="flex flex-col items-center gap-0.5">
                          ...
                          <span className="text-[10px] text-slate-400">{info.mesPico}</span>
                          <span className="text-[10px] text-slate-500 font-medium">{info.leads.toLocaleString()} leads</span>
                        </div>
                      ) : (
